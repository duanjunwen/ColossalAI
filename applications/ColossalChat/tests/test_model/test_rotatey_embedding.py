...
def run_qwen2_rotary_embedding():
    ...
    input_tensor_cuda = torch.ones(batch_size, seq_length, num_heads, head_dim, device="cuda")
    rotary_embedding_cuda = Qwen2RotaryEmbedding(config, device="cuda")
    positions_cuda = torch.arange(seq_length, device="cuda").unsqueeze(0).expand(batch_size, -1)
    ...
    torch.save(tensor_pt, f"./tests/tensor_log/cuda/RotaryEmbedding.pt")
...
def test_qwen2_rotary_embedding():
    ...
    if torch.cuda.is_available():
        ...
        rotary_embedding_cuda = rotary_embedding.cuda()
