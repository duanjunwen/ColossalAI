...
class ZeroBubbleVSchedule(PipelineSchedule):
    def __init__(
        self,
    ...
    def direction_map(self, microbatch_id: int, is_forward: bool) -> dict:
        node_chunk = self.get_model_chunk_id(microbatch_id, is_forward)
        if node_chunk == 0:
            return {
    ...
    def recv_forward(self, model_chunk_id: int, prev_rank: int = None) -> Tuple[Any, List]:
        ...
        with self.stage_manager.switch_model_chunk_id(model_chunk_id):
            if not self.stage_manager.is_first_stage():
                if model_chunk_id == 0:
                    # recv tensor from prev rank
                    prev_rank = self.stage_manager.get_prev_rank()
                    input_tensor, wait_handles = self.comm.recv_forward(
                        prev_rank, metadata_recv=self.tensor_metadata_recv
                    )
                else:
    ...
    def recv_backward(self, model_chunk_id: int, next_rank: int = None) -> Tuple[Any, List]:
        ...
        with self.stage_manager.switch_model_chunk_id(model_chunk_id):
            if not self.stage_manager.is_last_stage():
                if model_chunk_id == 0:
                    # recv bwd output_tensor_grad from next rank
                    next_rank = self.stage_manager.get_next_rank()
                    output_tensor_grad, wait_handles = self.comm.recv_backward(
                        next_rank, metadata_recv=self.grad_metadata_recv
                    )
                else:
                    # recv bwd output_tensor_grad from prev rank
                    prev_rank = self.stage_manager.get_prev_rank()
                    output_tensor_grad, wait_handles = self.comm.recv_backward(
                        prev_rank, metadata_recv=self.grad_metadata_recv
                    )
    ...
    def send_backward(self, model_chunk_id: int, input_tensor_grad: Any, prev_rank: int = None) -> List:
        ...
        with self.stage_manager.switch_model_chunk_id(model_chunk_id):
            if not self.stage_manager.is_first_stage():
                if model_chunk_id == 0:
                    # send input_tensor_grad to prev rank (common bwd)
                    prev_rank = self.stage_manager.get_prev_rank()
                    send_handles = self.comm.send_backward(
                        input_tensor_grad, prev_rank, send_metadata=self.send_grad_metadata
                    )
                else:
                    # send input_tensor_grad to prev rank (reverse bwd for chunk_id=1)
                    next_rank = self.stage_manager.get_next_rank()
                    send_handles = self.comm.send_backward(
    ...
    def run_forward_only(
        ...
        for _ in range(self.num_microbatch):
            model_chunk_id = self.get_model_chunk_id(_, is_forward=True)
            input_obj = self.recv_forward(model_chunk_id)
            output_obj = self.forward_step(model, model_chunk_id, input_obj, criterion, accum_loss, outputs)
    ...
    def run_forward_backward(
        ...
        num_warmup_microbatches = min(num_warmup_microbatches, self.num_microbatch)
        num_microbatches_remaining = self.num_microbatch - num_warmup_microbatches
